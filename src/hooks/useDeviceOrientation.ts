...
export const useDeviceOrientation = (): UseDeviceOrientationData => {
  ...
  const requestAccessAsync = async (): Promise<boolean> => {
    ...
    if (
      (DeviceOrientationEvent as unknown as DeviceOrientationEventWithPermission).requestPermission
      && typeof (DeviceMotionEvent as unknown as DeviceOrientationEventWithPermission).requestPermission === 'function'
    ) {
